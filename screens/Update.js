...
class Add extends React.Component {  
    ...
    <Block>
        ...
        <Block style={styles.textAreaContainer}>
            <TextInput
                editable={this.state.status_issue==0 ? true : false}
        ...
        <Text style={{marginTop: 10, marginBottom: -20}}>Badge</Text>
            {
                this.state.arr_employee.map((v, indx) => {
                    var x = this.state.arr_employee[indx].name
                    return (
                        <Block row key={indx}> 
                        <Block style={ indx==0 ? {width:'70%'} : {width:'87%'} }>
                        {/* <Block style={{ width: '100%' }}> */}
                            <Text
                            title={''}
                            size={15}
                            ...
                            editable={typeof x!=='undefined' ? false : true}
                            color="black"
                            placeholder="Badge Employee"
                            placeholderTextColor={nowTheme.COLORS.PLACEHOLDER}
                            defaultValue={this.state.arr_employee[indx].name}
                            onEndEditing={(val) => this.update_value_total_smoe(val, indx)}

                            />
                        </Block>
                        <Block style={ indx==0 ? {width:'30%'} : {width:'15%'} } row>
                            <Button 
                                onlyIcon
                                ...
                                iconColor="#fff"
                                style={{ width: 40, height: 40, marginTop: 30 }}
                                onPress={(i) => indx==0 ? this.add_employee() : (typeof x!=='undefined' ? this.showDelete(this.state.arr_employee[indx].id) : this.removeComponent(indx))}
                                >
                                warning
                            </Button>
                            {this.buttonFirstIndex(indx)}
                        </Block>
                        </Block>
                    )
                })
    ...
    onSubmit = async() => {
        if(this.state.status_issue!=0){
            //console.log(this.state.status_issue)
            //console.log('=====================')
            ...
            // return
            var url = config.config.url + '/api/iym/update_main';
            var api = this.state.dataLogin.api;
        ...
        var url_photo = config.config.url + '/api/iym/update_foto/'+this.state.running_number;
